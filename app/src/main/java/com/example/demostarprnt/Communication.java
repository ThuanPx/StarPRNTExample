...
public class Communication {
    ...
    public static void sendCommandsForRedirection(Object lock, byte[] commands, List<PortInfo> printerSettingsList, int timeout, Context context, PrintRedirectionCallback callback) {
...
// TODO change PrinterSettings -> PortInfo
class SendCommandForRedirectionThread extends Thread {
    private final Object                           mLock;
    private byte[]                                 mCommands;
    private List<PortInfo>                  mPrinterSettingsList;
    private Context                                mContext;
    private Communication.PrintRedirectionCallback mCallback;
    private int                                    mTimeout;

    SendCommandForRedirectionThread(Object lock, byte[] commands, List<PortInfo> printerSettingsList, int timeout, Context context, Communication.PrintRedirectionCallback callback) {
        mLock                = lock;
        mCommands            = commands;
    ...
    public void run() {
        ...
        synchronized (mLock) {
            ...
            for (PortInfo printerSettings : mPrinterSettingsList) {
        ...
        synchronized (mLock) {
            try {
                ...
                while (true) {
                    ...
                    if (mParser.parse(receiveData, receiveDataLength) == IPeripheralCommandParser.ParseResult.Success) {
                        result = true;
                        communicateResult = Communication.Result.Success;
                        break;
                    }
